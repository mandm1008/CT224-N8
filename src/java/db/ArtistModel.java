...
public class ArtistModel extends Model {
    ...
    public ArtistModel readResultSet(ResultSet rs) {
        try {
            ArtistModel am = new ArtistModel();

            am.artistId = rs.getInt("artist_id");
            am.name = rs.getString("name");
            am.genre = rs.getString("genre");
    ...
    public LinkedList<ArtistModel> search(String searchKey) {
        QueryResult qr = super.query("SELECT * FROM Artists WHERE name LIKE ?", (pstm) -> {
            try {
        ...
        LinkedList<ArtistModel> result = new LinkedList<>();
        ResultSet rs = qr.getResultSet();

        try {
            while (rs.next()) {
                ArtistModel am = readResultSet(rs);
                System.out.println(am.name);
                if (am != null) result.add(am);
            }
        } catch (SQLException ex) {
            ex.printStackTrace();
        }
        
        System.out.println(result.size() + " " + searchKey);
