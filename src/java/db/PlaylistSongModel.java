...
public class PlaylistSongModel extends Model {
    ...
    public boolean exists(int playlistId, int songId) {
        boolean found = false;
        try {
            QueryResult qr = super.query("SELECT * FROM " + getTableName() + " WHERE playlist_id = ? AND song_id = ?", (pstmt) -> {
                try {
                    pstmt.setInt(1, playlistId);
                    pstmt.setInt(2, songId);
                } catch (SQLException e) {
                    e.printStackTrace();
                    throw new RuntimeException("Lỗi khi đặt giá trị playlistId và songId: " + e.getMessage(), e);
                }
            });
            ResultSet rs = qr.getResultSet();
            found = rs.next();
            ...
            e.printStackTrace();
            throw new RuntimeException("Lỗi khi kiểm tra tồn tại của bài hát trong playlist: " + e.getMessage(), e);
        }
        return found;
    }
